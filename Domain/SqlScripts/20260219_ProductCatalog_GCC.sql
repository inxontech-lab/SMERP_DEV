...
BEGIN
    ...
    (
        SELECT 1
        FROM sys.columns c
        WHERE c.object_id = OBJECT_ID('dbo.Branches')
          AND c.name = 'IsWarehouse'
          AND c.is_nullable = 1
    )
    BEGIN
        UPDATE dbo.Branches
        SET IsWarehouse = ISNULL(IsWarehouse, 0)
        WHERE IsWarehouse IS NULL;

        ALTER TABLE dbo.Branches
        ALTER COLUMN IsWarehouse BIT NOT NULL;
    END;
    ...
    (
        ...
        WHERE dc.parent_object_id = OBJECT_ID('dbo.Branches')
          AND c.name = 'IsWarehouse'
    )
    BEGIN
        ALTER TABLE dbo.Branches
        ADD DEFAULT (0) FOR IsWarehouse;
    END;
END;
